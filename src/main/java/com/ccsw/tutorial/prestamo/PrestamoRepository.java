...
public interface PrestamoRepository extends CrudRepository<Prestamo, Long> {
     ...
     * @param prestamoId de la entidad
     * @return {boolean true si está prestado, false en caso contrario.
     */
    @Query("SELECT COUNT(p) > 0 FROM Prestamo p " + "WHERE p.game.id = :gameId " + "AND (:start <= p.fechaDevolucion AND :end >= p.fechaPrestamo)" + "AND (:prestamoId IS NULL OR p.id <> :prestamoId)")
    boolean isGameOccupied(@Param("gameId") Long gameId, @Param("start") LocalDate fechaPrestamo, @Param("end") LocalDate fechaDevolucion, @Param("prestamoId") Long prestamoId);
     ...
     */
    @Query("SELECT count(p) > 0 FROM Prestamo p " + "WHERE p.client.id = :clientId " + "AND (:start <= p.fechaDevolucion AND :end >= p.fechaPrestamo)" + "AND (:prestamoId IS NULL OR p.id <> :prestamoId)")
    boolean isClientOccupied(@Param("clientId") Long clientId, @Param("start") LocalDate fechaPrestamo, @Param("end") LocalDate fechaDevolucion, @Param("prestamoId") Long prestamoId);
